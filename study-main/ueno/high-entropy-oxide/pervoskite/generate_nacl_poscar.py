...
TOTAL_DATA = 10  # 生成するデータの総数
FILE_PATH = "ueno/study/high-entropy-oxide/rocksalt-NiMgCuCoZnO/scripts/generate-poscars"
FOLDER_NAME = "POSCARs"
...
def generate_poscar_and_combinations():
    ...
    reverse_existing_data, start_id = load_existing_data(f"{FILE_PATH}/data.csv")
    data_to_generate = TOTAL_DATA - len(reverse_existing_data)
    ...
    new_data = {}
    reverse_new_data = {}
    while len(new_data) < data_to_generate:
        ...
        if (
            feature_vector_str not in reverse_existing_data
            and feature_vector_str not in reverse_new_data
        ):
            new_data[start_id] = feature_vector_str
            reverse_new_data[feature_vector_str] = start_id
            poscar_file_path = os.path.join(
                FILE_PATH, FOLDER_NAME, f"POSCAR_{start_id}"
            )
            structure.to(fmt="poscar", filename=poscar_file_path)
            start_id += 1

    with open(f"{FILE_PATH}/data.csv", "a", newline="", encoding="utf-8") as csvfile:
